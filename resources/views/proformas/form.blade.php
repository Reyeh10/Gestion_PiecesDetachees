...
document.addEventListener(
    ...
    function () {
        ...
        function addRow(oldItem = null) {
            ...
            row.innerHTML = `
                <td>
                    <select name="items[${currentIndex}][product_id]" class="form-control product-select" required>
                        <option value="">Choisir un produit</option>
                        @foreach($products as $product)
                            <option value="{{ $product->id }}">
                                {{ $product->reference }} | {{ $product->designation }} | {{ $product->brand->name ?? '' }} | {{ $product->model->name ?? '' }}
                            </option>
                        @endforeach
                    </select>
                </td>
                <td class="text-center fw-bold">
                    <span id="stock_${currentIndex}">0.00</span><br>
                    <small id="stock_unit_${currentIndex}" class="text-muted">Pièce</small>
                </td>
                <td>
                    <select
                        name="items[${currentIndex}][depot_id]"
                        id="depot_${currentIndex}"
                        class="form-control depot-select"
                        required
                        disabled
                    >
                        <option value="">Sélectionnez d’abord un produit</option>
                    </select>
                    <small id="depot_stock_${currentIndex}" class="text-muted d-block mt-1"></small>
                </td>
                <td>
                    <input type="hidden" name="items[${currentIndex}][price]" id="price_${currentIndex}" value="0">
                    <div class="d-flex align-items-center gap-2 justify-content-end">
                        <div class="form-control price-display bg-white" id="price_display_${currentIndex}">0.00</div>
                        <span class="text-nowrap">FDJ</span>
                    </div>
                </td>
                <td>
                    <input
                ...
                <td class="text-end fw-bold">
                    <span id="total_${currentIndex}" class="line-total">0.00</span> FDJ
                </td>
            ...
            const depotStockInfo = row.querySelector(`#depot_stock_${currentIndex}`);

            function selectedProduct() {
                return productById(productSelect.value);
            }

            function selectedDepot() {
                const product = selectedProduct();
                if (!product || !depotSelect.value) {
                    return null;
                }

                return (product.depots || []).find(
                    depot => String(depot.depot_id) === String(depotSelect.value)
                ) || null;
            }
            ...
            function depotChanged() {
                ...
                if (!product || !depot) {
                    ...
                    depotStockInfo.textContent = '';
                    calculateRow(currentIndex);
                    return;
                }

                quantityInput.max = String(depot.quantity);
                depotStockInfo.textContent =
                    `Disponible dans ce dépôt : ${formatNumber(depot.quantity)} ${product.unit}`;

                const qty = parseFloat(quantityInput.value || 0);
                if (qty > parseFloat(depot.quantity || 0)) {
                    quantityInput.value = String(depot.quantity);
                }
            ...
            function productChanged() {
                const product = selectedProduct();
                const stockElement = document.getElementById(`stock_${currentIndex}`);
                const unitElement = document.getElementById(`stock_unit_${currentIndex}`);
                const priceInput = document.getElementById(`price_${currentIndex}`);
                const priceDisplay = document.getElementById(`price_display_${currentIndex}`);
                ...
                if (!product) {
                    ...
                    unitElement.textContent = 'Pièce';
                    priceInput.value = '0';
                    priceDisplay.textContent = '0.00';
                    quantityInput.removeAttribute('max');
                    depotSelect.disabled = true;
                    depotSelect.innerHTML = '<option value="">Sélectionnez d’abord un produit</option>';
                    calculateRow(currentIndex);
                    return;
                }

                stockElement.textContent = formatNumber(product.stock);
                unitElement.textContent = product.unit;
                priceInput.value = parseFloat(product.price || 0).toFixed(2);
                priceDisplay.textContent = formatNumber(product.price);

                const depots = product.depots || [];
                depotSelect.disabled = false;
                depotSelect.innerHTML = '<option value="">Sélectionner un dépôt</option>';

                depots.forEach(function (depot) {
                    const option = document.createElement('option');
                    option.value = String(depot.depot_id);
                    option.textContent = `${depot.name}${depot.code ? ' (' + depot.code + ')' : ''} — ${formatNumber(depot.quantity)} ${product.unit}`;
                    depotSelect.appendChild(option);
                });

                if (oldItem && oldItem.depot_id) {
                    depotSelect.value = String(oldItem.depot_id);
                }
            ...
            if (window.jQuery && $.fn.select2) {
                ...
                $(productSelect).on('change', productChanged);
            } else {
                productSelect.addEventListener('change', productChanged);
            }

            depotSelect.addEventListener('change', depotChanged);
            ...
            removeButton.addEventListener('click', function () {
                if (
                    window.jQuery &&
                    $.fn.select2 &&
                    $(productSelect).hasClass('select2-hidden-accessible')
                ) {
            ...
            if (oldItem && oldItem.product_id) {
                if (window.jQuery && $.fn.select2) {
                    ...
                    productSelect.value = String(oldItem.product_id);
                    productChanged();
                }
